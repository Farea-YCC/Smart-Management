...
// SQL statmant
String _purchasesTbl = 'CREATE TABLE ${DbTables.Purchases} (Id INTEGER PRIMARY KEY AUTOINCREMENT, productName TEXT, Quantity INTEGER, purchasePrice INTEGER, salePrice  INTEGER, Supplier  TEXT, amountPaid INTEGER, remainingAmount INTEGER, Discount INTEGER, dateOfAmount TEXT)';
...
// fun initialize Database
  Future<Database> _initializeDatabase() async {
    ...
    final file = File(p.join(dbFolderDir.path, 'Purchases.db'));
    var purchasesDb = await openDatabase(
        file.path,
        version: dbVersion,
  ...
  void _createDatabaseV1(Database db, int version) async {
    try {
      await db.execute(_purchasesTbl);
    }
    catch(e){
